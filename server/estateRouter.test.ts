...
const dbMock = vi.hoisted(() => ({
  createProperty: vi.fn(async () => undefined),
  deleteProperty: vi.fn(async () => undefined),
  updatePropertyStatus: vi.fn(async () => undefined),
  listFavoritePropertyIds: vi.fn(async () => [3, 8]),
...
const ctx = (role: "admin" | "visitor" | null): TrpcContext => ({ user: role ? { id: 1, name: "Test User", email: "test@example.com", role } : null, req: { protocol: "https", headers: {} } as TrpcContext["req"], res: { cookie: vi.fn(), clearCookie: vi.fn() } as unknown as TrpcContext["res"] });
...
describe("estate favorites procedures", () => {
  it("lists and changes favorites for a signed-in visitor", async () => {
    const caller = estateRouter.createCaller(ctx("visitor"));
    await expect(caller.favorites.list()).resolves.toEqual([3, 8]);
    await caller.favorites.add({ propertyId: 21 });
    await caller.favorites.remove({ propertyId: 21 });
    expect(dbMock.addFavorite).toHaveBeenCalledWith(1, 21);
    expect(dbMock.removeFavorite).toHaveBeenCalledWith(1, 21);
  });
  it("rejects favorites operations without a signed-in account", async () => {
    const caller = estateRouter.createCaller(ctx(null));
    await expect(caller.favorites.add({ propertyId: 21 })).rejects.toMatchObject({ code: "UNAUTHORIZED" });
    await expect(caller.favorites.remove({ propertyId: 21 })).rejects.toMatchObject({ code: "UNAUTHORIZED" });
